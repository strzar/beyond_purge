...
def eval_mia(model, tokenizer, forget, output_result_dir=None, use_prompt=False):
    ...
    for i, d in enumerate(tqdm(forget, total=len(forget))):
        ...
        token_log_probs = log_probs.gather(dim=-1, index=input_ids).squeeze(-1).float()
        mu = (probs * log_probs).sum(-1).float()
        sigma = ((probs * torch.square(log_probs)).sum(-1) - torch.square(mu)).float()
    ...
    return np.mean(scores['loss']), np.mean(scores['zlib']), np.mean(scores['mink++_0.2']), dict(scores)
