...
class BinaryReward(RewardFunction):
    ...
    def preprocess(cls, config: RewardConfig) -> None:
        ...
        cls.reset()
        cls._config = config
        cls._forget_words = cls.load_forget_words(config.forget_words_file, config.forget_set_filter)
        
        # Pre-compile regex pattern for efficiency
        cls._pattern = re.compile(
            r"\b(?:"
            + "|".join(map(re.escape, cls._forget_words))
            + r")\b",
            re.IGNORECASE,
        )
