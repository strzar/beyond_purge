#!/bin/bash
HF_ORG="${HF_ORG:?Set HF_ORG to your Hugging Face org/username, e.g. HF_ORG=your-org scripts/upload.sh}"
...
for target in "${names[@]}"; do
    ...
    python ../src/misc/huggingface_uploads/upload.py --target "$target" --hf-org "$HF_ORG"

    echo "Finished upload for: $target"
    echo "-------------------------------"
done
